...
┌─────────────────────────────────────────────────────────────────────┐
│                         pytest Runner                               │
│  conftest.py — browser fixture, auth_context, guest_page, 세션 관리   │
└───────────┬──────────────────────────────────┬──────────────────────┘
            │                                  │
            ▼                                  ▼
┌───────────────────────┐        ┌──────────────────────────┐
│    테스트 스위트         │        │   Page Object Model       │
│                       │        │                          │
│  test_login.py        │        │  BasePage                │
│  test_chatbot.py      │───────▶│   ├─ navigate, click     │
│  test_chatbot_learn.py│        │   ├─ search_stock        │
│                       │        │   └─ wait_for_element    │
└───────────┬───────────┘        │                          │
            │                    │  LoginPage               │
            │                    │   ├─ login_with_google    │
...
┌───────────────────────┐   REST API    ┌──────────────────┐
│  Evaluator            │──────────────▶│  Ollama          │
│  (ThreadPoolExecutor) │◀──────────────│  (Local LLM)     │
│  - 병렬 채점 (2 workers)│  1~5점 채점    │  qwen2.5:3b      │
└───────────┬───────────┘               └──────────────────┘
            │
            ▼
┌───────────────────────┐
│  Report Generator     │
│  - eval_NNN.json      │  개별 평가 결과
│  - evaluation_summary │  통합 요약
│  - report.html        │  시각화 리포트
└───────────────────────┘
```
